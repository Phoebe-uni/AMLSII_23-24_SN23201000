...
def compute_metrics(sr_imgs, hr_imgs, sr_type = '[0, 1]', hr_type = '[0, 1]') :
    """
    Calculate the PSNRs and SSIMs.

    :param sr_imgs:  source Images
    :param hr_imgs:  HiRes Images
    :param sr_type:  sr_image type: such as ‘[0, 1]’, '[-1, 1]','pil'...
    :param hr_type:  hr_image type.
    """

    # Calculate PSNR
    sr_imgs_y = convert_image(sr_imgs, source=sr_type, target='y-channel').squeeze(0)  # (w, h), in y-channel
    hr_imgs_y = convert_image(hr_imgs, source=hr_type, target='y-channel').squeeze(0)  # (w, h), in y-channel
...
def compute_psnr(image1, image2):
    """
    Calculate the psnr by two image
    """
    return compare_psnr(np.array(image1), np.array(image2), data_range=255)


def compute_ssim(image1, image2):
    """
    Calculate the ssim by two image
    """
    return SSIM(np.array(image1), np.array(image2), multichannel=True, channel_axis=2)