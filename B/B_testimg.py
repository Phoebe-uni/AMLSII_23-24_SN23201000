...
def show_testimg(serialno = "0814", rectno1 = 6, rectno2 = 66):
    ...
    #lr_dir = DATA_FOLDER / "DIV2K_train_LR_unknown" / "DIV2K_train_LR_unkown" / "X2" /f"{serialno}x2.png"
    #hr_dir = DATA_FOLDER / "DIV2K_train_HR" / f"{serialno}.png"

    sno = int(serialno)
    if sno > 800 :
        lr_dir = DATA_FOLDER / "DIV2K_valid_LR_unknown" / "DIV2K_valid_LR_unknown" / "X2" /f"{serialno}x2.png"
        hr_dir = DATA_FOLDER / "DIV2K_valid_HR" / f"{serialno}.png"
    else:
        lr_dir = DATA_FOLDER / "DIV2K_train_LR_unknown" / "DIV2K_train_LR_unknown" / "X2" /f"{serialno}x2.png"
        hr_dir = DATA_FOLDER / "DIV2K_train_HR" / f"{serialno}.png"

    ##checkpoint pth file.
    TASK_FOLDER = BASEDIR / "B" / "out"
    ...
    '''
    srresnet_ckpt = TASK_FOLDER / "B_ckpt_srresnet.pth.tar"
    srgan_ckpt = TASK_FOLDER / "B_ckpt_srgan.pth.tar"
    edsr_ckpt = TASK_FOLDER / "B_ckpt_edsr.pth.tar"
    srcnn_ckpt = TASK_FOLDER / "B_ckpt_srcnn.pth.tar"
    ...
    fig_file = TASK_FOLDER / f"B_TestImg({serialno}).png"
...
if __name__ == '__main__':
    ...
    #grid_img = show_testimg("0882", 13, 78)
    #grid_img = show_testimg("0826", 13, 78)
